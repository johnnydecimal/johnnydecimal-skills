...
- Looks for `.johnnydecimal` in your current folder.
  ...
  - Failing this, the skill will ask the user for an ID.
  - It has been instructed to **not guess** the ID. (But it's an LLM so, y'know.)
- Invoke the skill mentioning your JDex, e.g. `read my jdex and catch up`.
- All future documentation-related queries should use this skill. You shouldn't need to keep mentioning `jdex` (but it can't hurt).
