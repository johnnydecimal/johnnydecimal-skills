...
- The user can refer you to the JDex by way of a Johnny.Decimal ID.
  ...
  - Match the file using a glob from `$JDEX_ROOT`: e.g. `**/12.34*` for ID 12.34, or `**/90001*` for 5-digit ID 90001. Substitute the actual ID the user gives you.
...
- If it doesn't exist, check the current folder name for a Johnny.Decimal ID in square brackets, e.g. `my-project [12.34]` or `some-repo [90001]`. If found, treat it as the ID for this folder — same as if a dotfile had specified it.
